...
let treeDataProvider: EndpointsTreeDataProvider;
...
async function onOpenPort(port: Port): Promise<void> {
    ...
    if (port.interfaceListen !== LISTEN_ALL_IPV4 && port.interfaceListen !== LISTEN_ALL_IPV6) {
        const desc = `FIXME:::::::A new process is now listening on port ${port.portNumber} but is listening on interface ${port.interfaceListen} which is internal.
    ...
    if (matchingWorkspacePort) {
        ...
        const desc = `FIXME A new process is now listening on port ${port.portNumber} but this port is not exposed in the workspace as a server.
...
function freeRedirectPort(portNumber: number): void {
...
export async function start(context: theia.PluginContext): Promise<void> {
    ...
    // custom view
    treeDataProvider = new EndpointsTreeDataProvider(portChangesDetector, workspacePorts, redirectListeners, freeRedirectPort);
    ...
    theia.window.createTreeView('endpoints', { treeDataProvider });
